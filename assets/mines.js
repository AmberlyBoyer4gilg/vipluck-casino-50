...
return order;}
function buildGrid(){gridEl.innerHTML="";state.tiles=[];for(var i=0;i<TILE_COUNT;i+=1){var tile=document.createElement("button");tile.type="button";tile.className="tile-9c384be2";tile.disabled=true;tile.setAttribute("aria-label","Tile "+(i+1));tile.innerHTML='<span class="tile-inner-9c384be2">'+'<span class="tile-face-9c384be2 tile-front-9c384be2"></span>'+'<span class="tile-face-9c384be2 tile-back-9c384be2"></span>'+"</span>";tile.addEventListener("click",makeTileHandler(i));gridEl.appendChild(tile);state.tiles.push(tile);}}
function makeTileHandler(index){return function(){onTilePick(index);};}
function revealTile(index,isMine,dim){var tile=state.tiles[index];if(!tile||tile.classList.contains("revealed-9c384be2")){return;}
var back=tile.querySelector(".tile-back-9c384be2");if(back){back.innerHTML=isMine?BOMB_SVG:GEM_SVG;}
tile.classList.add("revealed-9c384be2");if(isMine){tile.classList.add("boom-9c384be2");}
if(dim){tile.classList.add("dimmed-9c384be2");}
tile.disabled=true;}
function revealRemaining(dimAll){for(var i=0;i<TILE_COUNT;i+=1){if(!state.tiles[i].classList.contains("revealed-9c384be2")){revealTile(i,state.mineMap[i]===true,dimAll);}}}
function setTilesEnabled(enabled){for(var i=0;i<TILE_COUNT;i+=1){var tile=state.tiles[i];tile.disabled=!enabled||tile.classList.contains("revealed-9c384be2");}}
...
var tile=state.tiles[index];if(!tile||tile.classList.contains("revealed-9c384be2")){return;}
...
function bust(){var lost=state.bet;revealRemaining(true);setStatus("Boom! The mine took your "+formatMoney(lost)+" bet.","lose");if(panelEl){panelEl.classList.remove("jolt-9c384be2");void panelEl.offsetWidth;panelEl.classList.add("jolt-9c384be2");}
endRound();}
